...
const ForgotPassword = () => {
  ...
  // Debug cuando hay error
  useEffect(() => {
    if (error) {
      console.log('🔥 Error detectado en ForgotPassword:', error)
    }
  }, [error])
  ...
  const handleEmailSubmit = async (e) => {
    e.preventDefault()
    console.log('🚀 HandleEmailSubmit ejecutado:', { email: formData.email })
    
    // Validar email
    const emailErrors = validateField('email', formData.email)
    if (Object.keys(emailErrors).length > 0) {
      console.log('❌ Errores de validación:', emailErrors)
    ...
    try {
      console.log('🔄 Llamando forgotPassword...')
      const result = await forgotPassword(formData.email)
      console.log('📥 Resultado recibido:', result)
      
      if (result && result.success) {
        console.log('✅ Email enviado exitosamente, cambiando a step token')
        setStep('token')
        
        // En desarrollo, mostrar el token en alerta
        if (result.resetToken) {
          console.log('🎯 Token recibido:', result.resetToken)
          alert(`Token de recuperación (solo desarrollo): ${result.resetToken}`)
        } else {
          console.log('⚠️ No se recibió token en la respuesta')
        }
      } else {
        console.log('❌ La respuesta no indica éxito:', result)
      }
    } catch (error) {
      console.error('💥 Error inesperado en handleEmailSubmit:', error)
    } finally {
      console.log('🏁 Finalizando handleEmailSubmit, isSubmitting = false')
  ...
  const handleResetSubmit = async (e) => {
    e.preventDefault()
    console.log('🚀 HandleResetSubmit ejecutado:', { token: formData.token })
    ...
    try {
      ...
      if (result.success) {
        console.log('✅ Contraseña reseteada, cambiando a step success')
        setStep('success')
      }
    } catch (error) {
        ...
        <div className="mb-8 z-20 text-center">
          <div className="flex flex-col items-center space-y-4">
            ...
            <div>
              ...
              <p className="text-sm text-blue-200">Universidad Colegio Mayor de Cundinamarca</p>
            </div>
          </div>
        </div>
